...
class bigindiclient: 
    ...
    def send_vector(self,vector):
        ...
        #************************************************************
        #*                   the old XML generation code.            *
        #************************************************************
        #it can be understood more easily as the new one and is thus kept here.
        #vectortag=vector._get_send_tag()
        #elementtag=vector.get_first_element()._get_send_tag()
        #data=""
        #data=data+"<"+vectortag+' device="'+vector.device+'" name="'+vector.name+'"> '
        #for i, element in enumerate(vector.elements):
        #   data=data+"<"+elementtag+' name="'+element.name
        #   if vector.type=="IBLOBVector":
        #       data=data+'" size="'+str(element.get_size())+'" format="'+element.format
        #   data=data+'"> '
        #   data=data+element._value
        #   data=data+"</"+elementtag+"> "
        #data=data+"</"+vectortag+"> "
        ...
        if True:
            ...
            if obj.tag.is_vector():
                ...
                if obj.tag.get_transfertype()==inditransfertypes.iset:
                    #print name,attrs
                    self.currentVector=self._get_and_update_vector(attrs,obj.tag)
            if self.currentVector!=None:
                if obj.tag.is_element():
                    if self.currentVector.tag.get_transfertype()==inditransfertypes.idef:
                        self.currentElement=obj
                    if self.currentVector.tag.get_transfertype()==inditransfertypes.iset:
                        self.currentElement=self._get_and_update_element(attrs,obj.tag)
                    if obj.tag.is_element():
                        self.currentData=""
        #except:
        #   a,b,c =sys.exc_info()
